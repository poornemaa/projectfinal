...
/*document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!token || !user_id || !user_type) {
    ...
    document.addEventListener('DOMContentLoaded', async () => {
      ...
      const user_id = localStorage.getItem('user_id');
      const user_type = localStorage.getItem('user_type');
      const guestView = localStorage.getItem('guest_view');  // Check if the admin is in guest view mode
    
      // Check for normal login or guest view
      if ((!token || !user_id || !user_type) && guestView !== 'true') {
        alert('Please login first.');
        window.location.href = '/index.html';  // Redirect to login page
        return;
      }
    
      // If admin is in guest view, they can access, but their actions will be limited
      if (guestView === 'true') {
        console.log('Admin is viewing the page in Guest View.');
        // Optional: disable actions or show a message that admin is in guest view
        messageDiv.innerHTML = 'You are in Guest View mode (Limited access)';
      } else {
        console.log('User is logged in normally.');
      }
    
      // Additional code for handling the Add Donor form, like event listeners, form validation, etc.
